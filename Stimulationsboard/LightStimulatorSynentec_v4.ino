...
void setup() {
  ...
  *dirUV   |= pinUV;
  ...
  //setup timer for UVoff
  int T1_PRESCALER = 256;
  int T1_PRELOAD = (unsigned short)(1.0 * F_CPU /T1_PRESCALER * uvFlash / 1000000);
  noInterrupts();  // protected code
  // reset Timer 1
  TCCR1A = 0;
  TCCR1B = 0; // No options in control register A
  TCCR1B = (1 << CS12); // Set prescaler to divide 256
  TIMSK1 = (1 << OCIE1A); // Call ISR when TCNT2 = OCRA2
  TCNT1=0;
  OCR1A = T1_PRELOAD; // Set frequency 
  interrupts();  // protected code     
}
...
ISR (TIMER1_COMPA_vect){
  TIMSK1 &= (0 << TOIE1); //disable timer overflow  
  lightOff();
}
...
void readSerial(MY_CMD cc){
  ...
  switch (cc.cCMD) {
      ...
      case 'F':        
        ...
        if (cc.nCount >0 ){
           Serial.print("Set uvFlash Duration to");
           Serial.println(cc.nData[0]*micro_to_milli);
           uvFlash=cc.nData[0];
        }
        bFlash=true;
        lightSwitchOff();
